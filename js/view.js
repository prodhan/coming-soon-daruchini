...
function drawScoreboard() {
	...
	if (gameState === 0) {
		...
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2.1 + gdy - 155 * settings.scale, 150, "#2c3e50", "দারুচিনি");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 10, fontSize, "rgb(44,62,80)", 'খেলুন!');
	} else if (gameState != 0 && textOpacity > 0) {
		textOpacity -= 0.05;
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy, 60, "rgb(236, 240, 241)", String.fromCharCode("0xf04b"), 'px FontAwesome');
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy - 155 * settings.scale, 150, "#2c3e50", "দারুচিনি");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h, fontSize, "rgb(44,62,80)", 'খেলুন!');
...
function showText(text) {
	var messages = {
		'paused': "<div class='centeredHeader unselectable'>খেলা আটকে গেছে!</div>",
		'pausedOther': "<div class='centeredHeader unselectable'>আটকে গেছে? </div><div class='unselectable centeredSubHeader' style='margin-top:10px;position:absolute;left:50%;margin-left:-190px;max-width:380px;font-size:18px;'><br> এই ফাঁকে একটা কথা বলি, দারুচিনি কিন্তু আসছে... <a href = 'https://www.pigeon-soft.com/' target='_blank'>ডেভেলপার খুঁজছেন? </a></div>",
		'start': "<div class='centeredHeader unselectable' style='line-height:80px;'>ছোঁয়া দিলেই খেলা শুরু হবে</div>"
	};
